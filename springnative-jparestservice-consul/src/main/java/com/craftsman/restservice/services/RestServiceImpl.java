...
public class RestServiceImpl implements IRestService {
	...
	private RestServiceDtoList readDelegate() throws RestServiceException {
		...
		try {
			Iterable<RestServiceBean> itEntities = repository.findAll();
			if (itEntities != null) {
				dtoList = new ArrayList<>();
				for (Iterator<RestServiceBean> iterator = itEntities.iterator(); iterator.hasNext();) {
					RestServiceBean e = (RestServiceBean) iterator.next();
	...
	private RestServiceDto readDelegate(Long id) throws RestServiceException {
		...
		try {
			Optional<RestServiceBean> e = repository.findById(id);
			if (e != null) {
				restServiceDto = mapper.mapRestServiceEntityToRestServiceDto(e.get());
			}
		} catch (Exception e) {
			log.error("Exception in method read", e);
			throw new RestServiceException("Exception in method read", e);
